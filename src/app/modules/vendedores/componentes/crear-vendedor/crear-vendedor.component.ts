...
export class CrearVendedorComponent {
  ...
  vendedorForm = new FormGroup({
    full_name: new FormControl<string>('', { validators: [Validators.required], nonNullable: true },),
    ...
    phone: new FormControl<string>('', { validators: [Validators.required, Validators.minLength(10), Validators.maxLength(10)], nonNullable: true }),
  ...
  isInvalid(controlName: string) {
    return this.vendedorForm.get(controlName)!.invalid &&
  ...
  crearVendedor() {
    ...
    this.vendedoresService.crearVendedor(vendedor).subscribe({
      next: (res) => {
        ...
        this.translate.get('VENDEDORES.CREAR_VENDEDOR.TOAST.SUCCESS').subscribe((mensaje: string) => {
